...
fn tokenize(chars: &[char]) -> Result<Vec<Token>, Error> {
    ...
    while i < chars.len() {
        match chars[i] {
            ...
            '(' => out.push(Token(
                Location::new(i, 1),
                TokenData::OpenBracket(Bracket::Round),
            )),
            '{' => out.push(Token(
                Location::new(i, 1),
                TokenData::OpenBracket(Bracket::Curly),
            )),
            '[' => out.push(Token(
                Location::new(i, 1),
                TokenData::OpenBracket(Bracket::Square),
            )),
            ')' => out.push(Token(
                Location::new(i, 1),
                TokenData::CloseBracket(Bracket::Round),
            )),
            '}' => out.push(Token(
                Location::new(i, 1),
                TokenData::CloseBracket(Bracket::Curly),
            )),
            ']' => out.push(Token(
                Location::new(i, 1),
                TokenData::CloseBracket(Bracket::Square),
            )),
            ' ' | '\t' | '\n' => {}
            _ => {
                let mut j = 1;
                ...
                let mut b = 1;
                let mut fail = false;
                for (i, c) in chars[i..i + j].iter().enumerate().rev() {
                    if i == 0 && *c == '-' {
                        a *= -1;
                ...
                out.push(Token(Location::new(i, j), data));
...
fn tokenizer_test() {
    ...
    test("a bb ty\"e b \" f s\" a", Err(Error::MissingEndQuote(Location::new(20, 0))));
